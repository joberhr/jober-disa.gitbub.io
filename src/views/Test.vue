  ...
  <div class="test">
    <div>
      <div class="answer-button-root">
        <div>
          <p style="font-weight: bold; font-size: 26px; margin-left: 4px; margin-bottom: 4px;">{{ currentProb }}번</p>
          <br />
          <div class="box">
            <div class="answer-button-wrapper">
              <button
                v-bind:class="{selected:currentButtonIdx===1}"
                class="answer-button"
                v-on:click="currentButtonIdx = 1"
              >
                {{ answerLists[(currentProb-1) * 4] }}
              </button>
              <button
                v-bind:class="{selected:currentButtonIdx===2}"
                class="answer-button"
                v-on:click="currentButtonIdx = 2"
              >
                {{ answerLists[((currentProb-1) *
                  4)+1] }}
              </button>
            </div>
            <div style="display: flex; flex-direction: row">
              <button
                v-bind:class="{selected:currentButtonIdx===3}"
                class="answer-button"
                v-on:click="currentButtonIdx = 3"
              >
                {{ answerLists[((currentProb-1) *
                  4)+2] }}
              </button>
              <button
                v-bind:class="{selected:currentButtonIdx===4}"
                class="answer-button"
                v-on:click="currentButtonIdx = 4"
              >
                {{ answerLists[((currentProb-1) *
                  4)+3] }}
              </button>
